...
def filter_subsumed_phrases(phrases_with_scores, dictionary):
    """
    더 긴 구에 포함된 짧은 구 제거 (선택적)
    예: "machine learning"이 있으면 "machine"은 제거
    """
    # 점수 기준 내림차순 정렬
...
def apply_ngram_weights(result, dictionary):
    """
    N-gram 길이에 따른 가중치 적용
    """
    weighted_scores = result['poster_pi_mn'].copy()
...
def print_results(result, cutoff, fdr_cutoff=FDR_CUTOFF, top_k=TOP_K_RESULTS):
    ...
    print(f"N-gram 가중치: 1-gram={NGRAM_WEIGHT[1]}, 2-gram={NGRAM_WEIGHT[2]}, 3-gram={NGRAM_WEIGHT[3]}")
    print()
    
    # N-gram 가중치 적용된 점수 계산
    weighted_scores = apply_ngram_weights(result, result['dictionary_minus'])
    
    # 가중치 적용된 cutoff 계산 (원래 cutoff에 평균 가중치 적용)
    avg_weight = np.mean(list(NGRAM_WEIGHT.values()))
    adjusted_cutoff = cutoff * avg_weight * 0.8  # 약간 낮춰서 더 많이 추출
    ...
    if MIN_NGRAM_OUTPUT > 1:
        phrases_with_scores = filter_by_ngram_length(
            phrases_with_scores, result['dictionary_minus'], MIN_NGRAM_OUTPUT
        )
    
    # 중복 제거 (더 긴 구에 포함된 짧은 구 제거)
    if EXCLUDE_SUBSUMED:
        phrases_with_scores = filter_subsumed_phrases(
            phrases_with_scores, result['dictionary_minus']
        )
    ...
    for i, (idx, score) in enumerate(sorted_phrases[:top_k], 1):
        phrase = result['dictionary_minus'][idx, 0]
        word_count = len(phrase.split())
        ngram_type = f"{word_count}-gram"
        original_score = result['poster_pi_mn'][idx]
        print(f"  {i:2d}. {phrase:40s} (가중:{score:.4f}, 원본:{original_score:.4f}) [{ngram_type}]")
    ...
    if len(sorted_phrases) > 0:
        ...
        # TextRank에도 n-gram 가중치 적용
        weighted_textrank = u_0_adjusted.copy()
        for idx in range(len(weighted_textrank)):
            phrase = result['dictionary_minus'][idx, 0]
            n = len(phrase.split())
            weighted_textrank[idx] *= NGRAM_WEIGHT.get(n, 1.0)
        
        textrank_scores = [(idx, weighted_textrank[idx]) for idx in range(len(weighted_textrank))]
        
        # 필터링 적용
        if MIN_NGRAM_OUTPUT > 1:
            textrank_scores = filter_by_ngram_length(
                textrank_scores, result['dictionary_minus'], MIN_NGRAM_OUTPUT
            )
        if EXCLUDE_SUBSUMED:
            textrank_scores = filter_subsumed_phrases(
                textrank_scores, result['dictionary_minus']
            )
        
        textrank_sorted = sorted(textrank_scores, key=lambda x: x[1], reverse=True)[:top_k]
        
        print(f"\n[TextRank 방법] - 상위 {top_k}개 (가중치 적용)")
        print("-" * 70)
        for i, (idx, score) in enumerate(textrank_sorted, 1):
            phrase = result['dictionary_minus'][idx, 0]
            word_count = len(phrase.split())
            ngram_type = f"{word_count}-gram"
            print(f"  {i:2d}. {phrase:40s} ({score:.4f}) [{ngram_type}]")
    
    # Semi-supervised 방법 (가중치 적용)
    if len(sorted_phrases) > 0:
        # Semi-supervised에도 n-gram 가중치 적용
        weighted_semi = result['Base_Line_minus'].copy()
        for idx in range(len(weighted_semi)):
            phrase = result['dictionary_minus'][idx, 0]
            n = len(phrase.split())
            weighted_semi[idx] *= NGRAM_WEIGHT.get(n, 1.0)
        
        semi_scores = [(idx, weighted_semi[idx]) for idx in range(len(weighted_semi))]
        
        # 필터링 적용
        if MIN_NGRAM_OUTPUT > 1:
            semi_scores = filter_by_ngram_length(
                semi_scores, result['dictionary_minus'], MIN_NGRAM_OUTPUT
            )
        if EXCLUDE_SUBSUMED:
            semi_scores = filter_subsumed_phrases(
                semi_scores, result['dictionary_minus']
            )
        
        semi_sorted = sorted(semi_scores, key=lambda x: x[1], reverse=True)[:top_k]
        
        print(f"\n[Semi-supervised 방법] - 상위 {top_k}개 (가중치 적용)")
        print("-" * 70)
    ...
    # N-gram 분포 통계 (BSS 결과 기준)
    print(f"\n{'='*70}")
    print("N-gram 분포 (BSS 결과)")
    print("="*70)
    ngram_counts = {1: 0, 2: 0, 3: 0}
    for idx, _ in sorted_phrases:
        phrase = result['dictionary_minus'][idx, 0]
        n = len(phrase.split())
