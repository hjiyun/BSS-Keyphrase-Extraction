...
# N-gram 가중치 (2-gram, 3-gram 선호 시 조절)
NGRAM_WEIGHT = {1: 1.0, 2: 1.2, 3: 1.3}  # n-gram별 점수 가중치

# 출력 옵션
PREFER_LONGER_PHRASES = True  # True면 더 긴 구를 선호
...
def print_results(result, cutoff, fdr_cutoff=FDR_CUTOFF, top_k=TOP_K_RESULTS):
    ...
    # 점수와 함께 저장
    phrases_with_scores = [(idx, result['poster_pi_mn'][idx]) for idx in identified_idx]
    ...
    if len(identified_idx) > 0:
        ...
        textrank_scores = [(idx, u_0_adjusted[idx]) for idx in range(len(u_0_adjusted))]
        textrank_sorted = sorted(textrank_scores, key=lambda x: x[1], reverse=True)[:top_k]
        
        print(f"\n[TextRank 방법] - 상위 {top_k}개")
        print("-" * 70)
    ...
    # Semi-supervised 방법
    if len(identified_idx) > 0:
        semi_scores = [(idx, result['Base_Line_minus'][idx]) for idx in range(len(result['Base_Line_minus']))]
        semi_sorted = sorted(semi_scores, key=lambda x: x[1], reverse=True)[:top_k]
        
        print(f"\n[Semi-supervised 방법] - 상위 {top_k}개")
        print("-" * 70)
