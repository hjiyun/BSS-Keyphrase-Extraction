...
# AWSGLD 파라미터
T = 5000              # 총 반복 횟수
BURN_IN = 500         # Burn-in 기간
TAU = 1.0             # Temperature parameter
ZETA = 5.0            # Adaptive weight scaling
SIGMA2_FIXED = 3.0   # Prior variance (고정)
...
ALPHA_FIXED = None  # seed ratio에서 자동 계산
...
def posterior_energy(Y, alpha, theta, u_0, B, sigma2):
    ...
    # Log-prior: -λ/2 * (θ - μ)^T L (θ - μ) where L = B^T B
...
def grad_posterior_energy(Y, alpha, theta, u_0, B, sigma2):
    """
    AWSGLD용 gradient of energy function
    ∇U(theta) = gradient of negative log posterior
    """
    n = len(theta)
    pi_theta = inv_logit(theta)
    pi_theta = np.clip(pi_theta, 1e-10, 1 - 1e-10)
    ...
    # Seeds (Y=1): ∂/∂θ log π(θ) = (1 - π(θ))
    if np.sum(Y == 1) > 0:
        grad_ll[Y == 1] = (1 - pi_theta[Y == 1])

    # Unlabeled (Y=0): ∂/∂θ log[(1-α)(1-π) + α·π]
    unlabeled_mask = (Y == 0)
    if np.sum(unlabeled_mask) > 0:
        prob_unlabeled = (1 - alpha) * (1 - pi_theta[unlabeled_mask]) + alpha * pi_theta[unlabeled_mask]
        prob_unlabeled = np.clip(prob_unlabeled, 1e-10, 1 - 1e-10)
        ...
        # ∂π/∂θ = π(1-π)
        dpi_dtheta = pi_theta[unlabeled_mask] * (1 - pi_theta[unlabeled_mask])

        # Chain rule: ∂/∂θ log(prob) = (∂prob/∂θ) / prob
        # where ∂prob/∂θ = [α - (1-α)] · π(1-π) = (2α-1) · π(1-π)
        grad_ll[unlabeled_mask] = ((2*alpha - 1) * dpi_dtheta) / prob_unlabeled

    # Gradient of log prior: -∂/∂θ [λ/2 (θ-μ)^T L (θ-μ)] = -λ L(θ-μ)
    BtB = B.T @ B
...
# ========== AWSGLD 샘플링 함수 ==========
def awsgld_sampler(Burn_in, T, ini, n, graph, Y, B, u_0, alpha_est,
    ...
    # AWSGLD adaptive weights 초기화 (논문 Algorithm 1: θ_0(i) = i/m)
    ...
    # Energy 범위 설정 (초기 100 iterations에서만 설정, 이후 고정)
    energy_samples = []
    warmup = 100
    energy_min = None
    energy_max = None
    ...
    for t in range(T):
        ...
        grad_U_tilde = grad_posterior_energy(Y, alpha_est, theta, u_0, B, sigma2)

        # ===== Energy range 설정 =====
        if t < warmup:
            # Warmup: energy 수집, adaptive weights 미사용
            energy_samples.append(U_tilde)
            J_tilde = m // 2
            grad_multiplier = 1.0
            if t == warmup - 1:
                e_min_raw = np.min(energy_samples)
                e_max_raw = np.max(energy_samples)
                e_range = max(e_max_raw - e_min_raw, 1.0)
                energy_min = e_min_raw - 0.5 * e_range
                energy_max = e_max_raw + 0.5 * e_range
                delta_u_actual = (energy_max - energy_min) / m
                if verbose:
                    print(f"\n  Energy 범위 설정 (고정): [{energy_min:.2f}, {energy_max:.2f}], delta_u={delta_u_actual:.4f}")
                energy_samples = None  # 메모리 해제
        else:
            # Subregion index 계산 (bounds 고정)
            J_tilde = int(np.clip(np.floor((U_tilde - energy_min) / delta_u_actual), 0, m - 1))

            # Gradient multiplier 계산 (adaptive weighting)
            eps_log = 1e-12
            grad_multiplier = 1 + (zeta * tau / delta_u_actual) * (
        ...
        # Clip weights (normalization 제거 - 논문에 없음)
...
def run_keyphrase_extraction(graph, obs_label, T, burn_in, alpha_fixed=0.5):
    ...
    ini = np.random.normal(0, 0.1, n_minus)
    alpha_est = alpha_fixed

    print(f"  AWSGLD 시작 (T={T}, Burn_in={burn_in})...")
    print(f"  AWSGLD 파라미터: tau={TAU}, zeta={ZETA}, alpha={alpha_fixed}, sigma2={SIGMA2_FIXED}")

    # AWSGLD 샘플러 사용
    test_chain = awsgld_sampler(burn_in, T, ini, n_minus, graph, Y_minus,
                                 B_minus, u_0_minus, alpha_est)
...
def print_and_save_results(result, cutoff, fdr_cutoff=FDR_CUTOFF, top_k=TOP_K_RESULTS):
    ...
    # ===== TextRank 방법 =====
    u_0_adjusted = force_obs_to_key(result['Y_minus'], result['u_0_minus'], 4)
...
if __name__ == "__main__":
    ...
    print(f"AWSGLD 파라미터: tau={TAU}, zeta={ZETA}, sigma2={SIGMA2_FIXED}, delta_u={DELTA_U}, m={M_REGIONS}")
    ...
    result = run_keyphrase_extraction(graph, OBS_LABEL, T, BURN_IN, alpha_fixed=ALPHA_FIXED)
    print("  추출 완료\n")
    ...
    predictions = [item['phrase'] for item in results_dict['methods']['BSS-AWSGLD']]
