...
def grad_posterior_energy(Y, alpha, theta, u_0, B, sigma2, batch_idx=None, BtB=None):
    """
    AWSGLD gradient of energy = -gradient of log posterior.

    batch_idx=None → full-batch likelihood (original behavior).
    batch_idx=array of node indices → minibatch likelihood gradient
        with unbiased scaling (n/batch_size). Prior gradient remains full
        because of graph coupling (B^T B is dense over 2-hop neighbors).

    BtB optional precomputed B.T @ B to avoid recomputing every step.
    """
    n = theta.shape[0]
    ...
    if batch_idx is None:
        ...
        # Minibatch likelihood, unbiased via scale = n / |batch|.
        scale = n / len(batch_idx)
        Y_b = Y[batch_idx]
        seed_b = batch_idx[Y_b == 1]
        unl_b = batch_idx[Y_b == 0]
        if seed_b.size > 0:
            grad_ll[seed_b] = (1 - pi_theta[seed_b]) * scale
        if unl_b.size > 0:
            grad_ll[unl_b] = -(1 - alpha) * dpi_dtheta[unl_b] / denom[unl_b] * scale
...
def gibbs_mh(Burn_in, T, ini, n, graph, Y, B, u_0, alpha_est, grid,
    ...
    batch_size=None → full-batch likelihood (default, identical to awsgld_0422).
    ...
    for t in range(T):
        ...
        grad_U = grad_posterior_energy(
            Y, alpha_est, theta, u_0, B, sigma2,
            batch_idx=batch_idx, BtB=BtB_fixed,
        )
