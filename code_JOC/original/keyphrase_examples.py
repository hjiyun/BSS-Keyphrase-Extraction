...
def big_graph_generate(article, base_dir="/users/wangguanshen/Desktop/keyphrase/SemEval2010/train_preprocess"):
...
def FDR_cutoff(poster_pi_md, c, Y, truth):
    ...
    FDR_pos = np.sum(poster_md_adjust >= cutoff)
    FDR_tp = np.sum(np.where(poster_md_adjust >= cutoff)[0] in truth) if len(truth) > 0 else 0
...
if Ans1 is not None:
    # Save result
    save_path = "/users/guanshenw/scratch/keyphrase/biggraphC42(sub_title).npz"
