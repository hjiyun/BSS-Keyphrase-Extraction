...
### 2.1 병합 문서에서의 6-sampler 통일 비교 (`merge_unified.py`)

§2 는 병합 사후분포가 단봉임을 보였다. 그 위에서 **샘플러들이 어떻게 다른가**를 study_a0/1A/1C
와 **동일한 지표 세트**로 본다. 실데이터라 연속 θ\* 가 없어(binary 키워드 gold 만 존재) 지표를
적응한다.

**구성** — 고중첩 문서를 탐욕적으로 병합(문서 내 window-2 동시출현만 합산, 문서 간 인위 간선
없음 → 고립노드 제거). B = I − d·(D⁻¹A)ᵀ (d = 0.85), u_0 = solve(B, (1−d)**1**), Y = 문서별 키워드
20% 관측, truth = 문서별 키워드 합집합, α̂ = alpha_find(u_0). **조건당 서로 다른 문서세트 10종**
(총중첩 순위대로 시작문서를 바꿔 탐욕 생성하되 **집합 기준 distinct** 10개 확보 — 각 세트가 최소
1편 다름)을 만들어 **그 10종을 평균**낸다.

| 항목 (10세트 평균) | 3편 병합 | 10편 병합 |
|---|---|---|
| n(병합 어휘) / truth(gold) | ~172 / ~75 (44%) | ~477 / ~215 (45%) |
| 관측 Y=1 (키워드 20%) | ~15 | ~43 |
| u_0 mean / α̂ / degree(평균) | 1.00 / 0.88 / 8.2 | 1.00 / 0.87 / 8.0 |

**setup** — 6 샘플러(acMH 포함), **4 연쇄**(init −0.5/0.5/1.5/random), **10 문서세트 평균**,
AWSGLD R̂median<1.05 조기종료(T_MAX 8000). acMH 는 원본 componentwise(소형 n 이라 CPU).
지표: **표1** split-R̂ med/q95/max · ESS · θ̂(kw/non-kw) · Lowest U, **표2(gold)** P@k · R@k ·
NDCG@k · AUC (k = |truth|). 아래는 10세트 mean±std.

**표1 (수렴·추정)** — θ̂ 는 키워드 gold vs 비키워드 그룹 평균

| 조건 | 샘플러 | R̂med | R̂max | ESS | θ̂(kw/non) | LowU |
|---|---|---|---|---|---|---|
| 3편 | acMH | 1.08±.02 | 1.39 | 17.9 | 1.42/0.81 | 97 |
| | SGLD | 3.02±.14 | 7.23 | 6.0 | 0.41/0.27 | 129 |
| | qSGLD | 1.29±.07 | 1.88 | 9.1 | 1.58/0.77 | 149 |
| | cycSGLD | 1.72±.20 | 5.56 | 12.8 | 0.64/0.27 | **62** |
| | SGHMC | 1.85±.09 | 4.52 | 6.9 | 0.57/0.27 | 126 |
| | **AWSGLD** | **1.08±.03** | **1.36** | **59.6** | 0.99/0.48 | **62** |
| 10편 | acMH | 1.22±.03 | 1.94 | 9.5 | 1.43/0.77 | 395 |
| | SGLD | 2.94±.06 | 8.72 | 6.0 | 0.37/0.22 | 390 |
| | qSGLD | 1.26±.04 | 2.01 | 9.3 | 1.52/0.72 | 469 |
| | cycSGLD | 1.49±.08 | 6.55 | 14.6 | 0.58/0.21 | **200** |
| | SGHMC | 1.78±.04 | 5.66 | 6.9 | 0.49/0.20 | 386 |
| | **AWSGLD** | **1.17±.07** | **1.87** | **49.7** | 0.99/0.47 | **200** |

**표2 (gold 순위, k=|truth|)**

| 조건 | 샘플러 | P@k | R@k | NDCG@k | AUC |
|---|---|---|---|---|---|
| 3편 | acMH | 0.645±.035 | 0.645 | 0.695 | 0.725 |
| | SGLD | 0.498±.041 | 0.498 | 0.532 | 0.572 |
| | qSGLD | 0.614±.062 | 0.614 | 0.672 | 0.708 |
| | cycSGLD | 0.637±.060 | 0.637 | 0.702 | 0.739 |
| | SGHMC | 0.544±.051 | 0.544 | 0.606 | 0.630 |
| | **AWSGLD** | **0.658±.050** | **0.658** | **0.712** | **0.748** |
| 10편 | acMH | 0.619±.025 | 0.619 | 0.658 | 0.701 |
| | SGLD | 0.516±.044 | 0.516 | 0.538 | 0.585 |
| | qSGLD | 0.639±.026 | 0.639 | 0.676 | 0.719 |
| | cycSGLD | 0.663±.027 | 0.663 | **0.706** | **0.754** |
| | SGHMC | 0.566±.035 | 0.566 | 0.600 | 0.640 |
| | **AWSGLD** | **0.667±.026** | **0.667** | 0.702 | 0.752 |

> **정리** — 단봉 실데이터에서도, **10종 문서세트 평균에서** AWSGLD 는 **수렴(R̂med 최소)·효율
> (ESS 50~60 vs 6~18)·최저 에너지**를 모두 잡고 키워드를 비키워드보다 높게 추정한다. **gold 추출
> 품질도 AWSGLD 가 P@k 를 두 조건 모두 1위**(0.658 / 0.667)이며, NDCG·AUC 는 cycSGLD 와 접전
> (3편 AWSGLD 1위, 10편 cycSGLD 가 0.004 차 근소 우위)이다. §1·§2 와 일관되게 AWSGLD 의 이득은
> mode escape 가 아니라 **넓은 탐색 + 높은 표본 효율**에서 나온다.
