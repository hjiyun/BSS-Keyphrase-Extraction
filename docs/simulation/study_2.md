...
지표: **표1** split-R̂ med/q95/max · ESS · θ̂(kw/non-kw) · **π̂(kw/non-kw)** · Lowest U,
**표2(gold)** P@k · R@k · NDCG@k · AUC (k = |truth|). 아래는 10세트 mean±std.

> **지표 대응 각주** — 합성 실험(1A/A0/1B/1C)은 연속 θ\* 가 있어 순위를 Spearman·Kendall·NDCG·
> Top-k 로 본다. merge 는 실문서라 연속 θ\* 가 없고 binary 키워드 gold 만 있어 **P@k·R@k·NDCG·
> AUC** 로 대응한다(Top-k=P@k, NDCG 는 공통). 따라서 Spearman·Kendall·MSE_θ 는 merge 에서
> 구조적으로 계산 불가, AUC 는 연속 θ\* 없는 실데이터 전용이다 — 지표 세트 차이는 데이터
> 특성에 따른 것이며 표 구조(표1 수렴·추정 / 표2 순위·추출)는 동일하다.

**표1 (수렴·추정)** — θ̂·π̂ 는 키워드 gold vs 비키워드 그룹 평균

| 조건 | 샘플러 | R̂med | R̂q95 | R̂max | ESS | θ̂(kw/non) | π̂(kw/non) | LowU |
|---|---|---|---|---|---|---|---|---|
| 3편 | acMH | 1.08 | 1.22 | 1.39 | 17.9 | 1.42/0.81 | .77/.68 | 97 |
| | SGLD | 3.02 | 5.41 | 7.23 | 6.0 | 0.41/0.27 | .59/.56 | 129 |
| | qSGLD | 1.29 | 1.63 | 1.88 | 9.1 | 1.58/0.77 | .77/.66 | 149 |
| | cycSGLD | 1.72 | 3.34 | 5.56 | 12.8 | 0.64/0.27 | .64/.56 | **62** |
| | SGHMC | 1.85 | 3.09 | 4.52 | 6.9 | 0.57/0.27 | .62/.56 | 126 |
| | **AWSGLD** | **1.08** | **1.22** | **1.36** | **59.6** | 0.99/0.48 | .70/.61 | **62** |
| 10편 | acMH | 1.22 | 1.53 | 1.94 | 9.5 | 1.43/0.77 | .75/.67 | 395 |
| | SGLD | 2.94 | 5.32 | 8.72 | 6.0 | 0.37/0.22 | .59/.55 | 390 |
| | qSGLD | 1.26 | 1.60 | 2.01 | 9.3 | 1.52/0.72 | .76/.65 | 469 |
| | cycSGLD | 1.49 | 3.04 | 6.55 | 14.6 | 0.58/0.21 | .63/.55 | **200** |
| | SGHMC | 1.78 | 2.97 | 5.66 | 6.9 | 0.49/0.20 | .61/.55 | 386 |
| | **AWSGLD** | **1.17** | **1.46** | **1.87** | **49.7** | 0.99/0.47 | .70/.61 | **200** |
