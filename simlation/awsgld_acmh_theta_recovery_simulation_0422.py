...
def summarize_estimates(method, theta_star, pi_star, theta_hat, pi_hat, alpha_hat, Y):
...
def run_acmh(graph, Y, theta_star, pi_star, init_state):
    ...
    return summarize_estimates("acMH", theta_star, pi_star, theta_hat, pi_hat, alpha_hat, Y)
...
def run_awsgld(graph, Y, theta_star, pi_star, init_state):
    ...
    pi_hat = (1.0 - alpha_hat) * inv_logit(theta_hat)
    pi_hat = np.clip(pi_hat, 1e-10, 1 - 1e-10)
    return summarize_estimates("AWSGLD", theta_star, pi_star, theta_hat, pi_hat, alpha_hat, Y)
...
def main():
    ...
    json_path = os.path.join(out_dir, "awsgld_acmh_theta_recovery_summary.json")
    png_path = os.path.join(out_dir, "awsgld_acmh_theta_recovery_summary.png")
    ...
    for r in range(R):
        ...
        print(
            ...
            f"MSE(pi)={acmh_result.mse_pi:.4f} | "
    ...
    for method, summary in summaries.items():
        print(
            f"[{method}] Mean MSE(theta)={summary['mse_theta']['mean']:.4f} "
